...
const updateGame = async (req: Request, res: Response) => {
  const updateGame = await gamesRepositories.updateGameQuerry(req.body, req.params.id)
  res.send(updateGame);
};
...
const getImagesForGame = async (req: Request, res: Response) =>{
  const getImagesForGame = await gamesRepositories.getImagesForGame(req.params.id);
  res.send(getImagesForGame);
}

export default { getAllGames, getGameById, insertGame, updateGame, deleteGame, getImagesForGame };
